...
DATA_PATH = r"C:\Datasets\Audio\Wav/"
SAVE_PATH = r"C:\Datasets\Audio/"
...
def save_data_to_array(path=DATA_PATH, max_len=11, n_mfcc=20):
    ...
    for label in labels:
        ...
        np.save(SAVE_PATH + label + '.npy', mfcc_vectors)
...
def get_train_test(split_ratio=0.6, random_state=42):
    ...
    X = np.load(SAVE_PATH + labels[0] + '.npy')
    y = np.zeros(X.shape[0])

    # Append all of the dataset into one single array, same goes for y
    for i, label in enumerate(labels[1:]):
        x = np.load(SAVE_PATH + label + '.npy')
        X = np.vstack((X, x))
